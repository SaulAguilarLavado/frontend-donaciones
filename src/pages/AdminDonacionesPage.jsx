...
export default function AdminDonacionesPage() {
  ...
  // MAPEO COLORES DE ESTADOS
  const getStatusBadge = (status) => {
    switch (status) {
      case "PENDIENTE":
        return "bg-secondary";
      case "RECOJO_ASIGNADO":
            ...
            <tbody>
              {donaciones.map(d => (
                <tr key={d.id}>
                  ...
                  {/* ACCIONES → solo si no está finalizado */}
                  <td>
                    {d.status !== "COMPLETADA" && d.status !== "CANCELADO" && (
                      <>
                        <button
                        ...
                        <button
                          ...
                          onClick={() => cambiarEstado(d.id, "CANCELADO")}
                        >
                          Cancelar
                        </button>
                      </>
